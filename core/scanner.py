...
class Scanner:
    ...
    def _analyze_symbol(self, symbol: str, interval: str, period: str) -> Optional[Dict]:
        ...
        close_prev = indicators["close"][idx - 1]   # Previous candle close (for crossover detection)
        ma20 = indicators["ma20"][idx]
        ma20_prev = indicators["ma20"][idx - 1]     # Previous candle MA20 (for crossover detection)
        ...
        # If the candle's open_time is older than 1.5 periods, the signal is stale.
        try:
            ...
            age_seconds = (now_utc - candle_open_dt).total_seconds()
            # Allow up to 1.5 periods (candle duration + 0.5 buffer for execution)
            max_age_seconds = period_seconds * 1.5
            if age_seconds > max_age_seconds:
                reason = (f"K线信号已过期: 信号K线开盘于 {candle_open_dt.strftime('%H:%M')} UTC, "
                          f"已过 {age_seconds/3600:.1f}h, 超过 {max_age_seconds/3600:.1f}h 限制")
                logger.warning(f"{symbol} {period}: Stale candle signal (age={age_seconds:.0f}s > max={max_age_seconds:.0f}s), skipping")
                add_log("warning", "SCANNER", f"{symbol} {period}: {reason}")
                # Note: direction not yet determined at this point, use placeholder
                signal_data = self._build_signal(symbol, period, "UNKNOWN",
                                                  "filtered", close, ma20, macd_hist,
                                                  0, atr_val if not np.isnan(atr_val) else 0,
                                                  rsi_val if not np.isnan(rsi_val) else 50, reason)
        ...
        # === CONDITION 1: Price Position (MA20 Crossover) ===
        # LONG: Previous candle close <= MA20, current candle close > MA20 (just crossed above)
        # SHORT: Previous candle close >= MA20, current candle close < MA20 (just crossed below)
        long_cond_price = (close_prev <= ma20_prev) and (close > ma20)
        short_cond_price = (close_prev >= ma20_prev) and (close < ma20)
        ...
        #   c) Current histogram < previous histogram (red bar growing = abs value increasing)
        long_cond_macd = (
            (dif > dea) and              # a) DIF above DEA (golden cross state)
            (macd_hist > 0) and          # b) Histogram is positive (green bar)
            (macd_hist > macd_hist_prev) # c) Green bar is growing (1 increase)
        )
        short_cond_macd = (
            (dif < dea) and              # a) DIF below DEA (death cross state)
            (macd_hist < 0) and          # b) Histogram is negative (red bar)
            (macd_hist < macd_hist_prev) # c) Red bar is growing in magnitude (1 increase)
        )
