...
class DTRSScheduler:
    ...
    async def _wait_until_next(self, interval_seconds: int, offset_seconds: int = 0):
        """Wait until the next interval boundary PLUS offset (scan AFTER candle close)"""
        now = datetime.now(timezone.utc)
        # Calculate seconds since epoch
        epoch_seconds = now.timestamp()
        # Time until next interval boundary (candle close moment)
        elapsed = epoch_seconds % interval_seconds
        # Wait until candle close, then add offset_seconds buffer to ensure candle is fully settled
        wait_time = interval_seconds - elapsed + offset_seconds

        # Ensure minimum wait of 30 seconds to prevent double-triggering
        # (e.g. if called right after a scan completes near the boundary)
        if wait_time <= 30:
            wait_time += interval_seconds

        logger.debug(f"Waiting {wait_time:.1f}s until next {interval_seconds}s interval (+{offset_seconds}s post-close buffer)")
        await asyncio.sleep(wait_time)

    async def _scan_loop_1h(self):
        """Scan 1h timeframe every hour, 5s AFTER candle close (ensures previous candle is fully settled)"""
        while self.running:
            try:
